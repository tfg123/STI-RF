from dataset import STKG
from model import STIRF
...
from read_tokens import get_entity_visual_tokens, get_entity_textual_tokens, get_entity_neighbor
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_epoch', default=1550, type=int)
    parser.add_argument('--valid_epoch', default=50, type=int)
    parser.add_argument('--exp', default='stirf')
    ...
    parser.add_argument('--emb_dropout', default=0.6, type=float)
    parser.add_argument('--vis_dropout', default=0.3, type=float)
    ...
    parser.add_argument('--max_vis_token', default=12, type=int)
    parser.add_argument('--max_txt_token', default=24, type=int)
    parser.add_argument('--neighbors', default=1, type=int)
    ...
    logger.info(args)
    KG = STKG(args.data, logger, max_vis_len=args.max_img_num)
    ...
    neighbors_index = get_entity_neighbor(dataset=args.data, max_num=args.neighbors)
    ...
    model = STIRF(
        ...
        neighbor_token_index=neighbors_index,
    ).cuda()

    
    loss_fn_s = nn.CrossEntropyLoss()  # label_smoothing=args.smoothing
    loss_fn_i = nn.CrossEntropyLoss()
    ...
    for epoch in range(last_epoch + 1, args.num_epoch + 1):
        ...
        for batch, label in KG_Loader:
            ...
            scores = model.score(ent_embs, rel_embs, batch.cuda(), label.cuda())
            loss = loss_fn_s(scores[0], label.cuda()) + loss_fn_i(scores[1], label.cuda()) + loss_fn_t(scores[2], label.cuda()) + loss_fn_m(scores[3], label.cuda()) 
